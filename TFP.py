...
# ------------------------------------------------------------------------------
# 1b) ตัวแปร "สำรอง" (candidate variables) — มีข้อมูลโหลด + แปลง log ไว้พร้อมแล้ว
# (เพราะ PCT_GDP/PATENT_GDP อยู่ใน LOG_VARS ด้านบนอยู่แล้ว) แต่ยังไม่ถูกใส่เข้า
# สมการจริงในเวอร์ชันปัจจุบัน (ไม่อยู่ใน LONG_RUN_VARS/SHORT_RUN_SPEC ข้างบน)
# ตัวแปรกลุ่มนี้จะไปโผล่เป็นตัวเลือกเพิ่มเติมในส่วน "ปรับตัวแปรในสมการ" ของหน้าเว็บ
# (app.py) — นักวิจัยเปิด/ปิดตัวแปรกลุ่มนี้ได้จากหน้าเว็บโดยตรง ไม่ต้องแก้ไฟล์นี้
#
# ข้อควรระวัง: diff_order/lag ที่ตั้งไว้ (1, 0) เป็นแค่ค่าเริ่มต้นอย่างง่าย (△
# ธรรมดา ไม่มี lag) ยังไม่ได้ผ่านการทดสอบ ADF/ทฤษฎีอย่างจริงจังว่าเหมาะสมหรือไม่
# — ก่อนเชื่อผลจริงจังควรดูตาราง Diagnostics (Stationarity/VIF) ที่หน้าเว็บสร้าง
# ให้อัตโนมัติหลังเปิดใช้ตัวแปรนี้ แล้วปรับ diff_order/lag ที่นี่ตามความเหมาะสม
#
# ถ้าจะเพิ่มตัวแปรใหม่ที่ "ยังไม่มีข้อมูลเลย" (ไม่ใช่ PCT_GDP/PATENT_GDP) ต้องแก้
# RAW_TO_MODEL / PERCENT_VARS / LOG_VARS ด้านบนก่อน (และเพิ่มคอลัมน์ข้อมูลใน
# Google Sheet ต้นทางให้ตรงกับ RAW_TO_MODEL) แล้วค่อยมาเพิ่มชื่อในนี้ทีหลัง
CANDIDATE_LONG_RUN_VARS = ["ln_PCT_GDP", "ln_PATENT_GDP"]

CANDIDATE_SHORT_RUN_SPEC = [
    ("ln_PCT_GDP", 1, 0),
    ("ln_PATENT_GDP", 1, 0),
]
